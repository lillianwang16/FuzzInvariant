...
static void write_for_daikon(void* mem, u32 len) {
  ...
  if (daikon_file) {
    ...
    unlink(daikon_file); /* Ignore errors. */

    fd = open(daikon_file, O_WRONLY | O_CREAT | O_EXCL, 0600);

    if (fd < 0) PFATAL("Unable to create '%s'", daikon_file);

  } else lseek(fd, 0, SEEK_SET);

  ck_write(fd, mem, len, daikon_file);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q")) > 0)
	...
	//TODO::Kathy
        daikon_file = alloc_printf("%s/daikon_input", out_dir);
        //printf(daikon_file);
	break;
